...
// GET /api/cron/inbox - Cron para sincronizar inbox a cada 5 minutos
export async function GET(req: NextRequest) {
  try {
    // 1. Validar header de autorização (Vercel Cron passa Bearer token)
    const authHeader = req.headers.get('authorization') || '';
    const cronSecret = process.env.CRON_SECRET;

    // Se CRON_SECRET não está setado, permitir em dev (não vai rodar em prod sem secret)
    ...
    // 3. Para cada conta, sincronizar inbox
    for (const account of accounts) {
      try {
        ...
        // 4. Para cada conversa, upsert no Supabase (dedupe por zernio_conversa)
    ...
    console.error('❌ Cron inbox error:', message);
    return NextResponse.json(
